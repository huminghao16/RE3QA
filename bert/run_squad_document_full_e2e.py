...
from bert.custom_modeling import BertForRankingAndReadingAndReranking
from bert.optimization import BERTAdam
from squad.squad_document_utils import read_squad_doc_examples, SquadTfIdfRanker, convert_examples_to_features, \
    RawRankResult, RawReadResult, RawFinalResult, write_rerank_predictions, eval_ranking, random_filter_features, \
    annotate_candidates
from squad.squad_evaluate import evaluate as eval_fn
...
def down_sample(sample_rate, features, logger):
    ...
    logger.info("Num pos features = %d", len(positive_features))
    logger.info("Num neg features = %d", len(negative_features))
    sample_number = int(sample_rate * len(positive_features))
    if sample_number < len(negative_features):
        sampled_negative_features = random.sample(negative_features, sample_number)
        logger.info("Num sampled neg features = %d", len(sampled_negative_features))
        return positive_features + sampled_negative_features
    else:
        return features
...
def read_train_data(args, tokenizer, logger):
    ...
    if os.path.exists(train_examples_path):
        ...
        train_documents = read_squad_doc_examples(input_file=train_path, logger=logger, debug=args.debug)

        preprocesser = SquadTfIdfRanker(tokenizer, n_to_select=args.n_para_train, is_training=True, sort_passage=True)
        train_examples = preprocesser.ranked_questions(train_documents)
...
def build_train_data(args, train_examples, train_features, filtered_train_features, logger):
    ...
    train_rank_dataloader = DataLoader(train_rank_data, sampler=train_rank_sampler, batch_size=train_batch_size_for_rank)
    ...
    all_start_positions = torch.tensor([f.start_position for f in filtered_train_features], dtype=torch.long)
    all_end_positions = torch.tensor([f.end_position for f in filtered_train_features], dtype=torch.long)
    all_example_index = torch.arange(all_input_ids.size(0), dtype=torch.long)
...
def read_eval_data(args, tokenizer, logger):
    eval_examples_path = os.path.join(args.data_dir, "eval_{}paras_examples.pkl".format(args.n_para_predict))
    if os.path.exists(eval_examples_path):
        eval_examples = pickle.load(open(eval_examples_path, 'rb'))
        logger.info("Loading examples from: %s" % (eval_examples_path))
    else:
        predict_path = os.path.join(args.data_dir, args.predict_file)
        eval_documents = read_squad_doc_examples(input_file=predict_path, logger=logger, debug=args.debug)

        preprocesser = SquadTfIdfRanker(tokenizer, n_to_select=args.n_para_predict, is_training=False,
                                        sort_passage=True)
        eval_examples = preprocesser.ranked_questions(eval_documents)
        pickle.dump(eval_examples, open(eval_examples_path, 'wb'))

    eval_features_path = os.path.join(args.data_dir, "eval_{}paras_{}max_{}stride_features.pkl"
                                      .format(args.n_para_predict, args.max_seq_length, args.doc_stride))
...
def read_eval_open_data(args, tokenizer, logger):
    eval_examples_path = os.path.join(args.data_dir, "eval_open_{}paras_examples.pkl".format(args.n_para_predict))
    assert os.path.exists(eval_examples_path)
    eval_examples = pickle.load(open(eval_examples_path, 'rb'))
    logger.info("Loading examples from: %s" % (eval_examples_path))

    eval_features_path = os.path.join(args.data_dir, "eval_open_{}paras_{}max_{}stride_features.pkl"
                                      .format(args.n_para_predict, args.max_seq_length, args.doc_stride))
    if os.path.exists(eval_features_path):
        eval_features = pickle.load(open(eval_features_path, 'rb'))
        logger.info("Loading features from: %s" % (eval_features_path))
...
def evaluate_read(args, model, device, eval_examples, eval_features, eval_rank_logits, eval_dataloader,
                  logger, write_pred=False, verbose_logging=False):
    all_results = []
    for input_ids, input_mask, segment_ids, example_indices in eval_dataloader:
        if len(all_results) % 5000 == 0 and verbose_logging:
            logger.info("Processing example: %d" % (len(all_results)))
    ...
    predict_path = os.path.join(args.data_dir, args.predict_file)
    with open(predict_path, "r") as reader:
        input_data = json.load(reader)["data"]
    metrics = eval_fn(input_data, all_predictions)
    return metrics
...
def run_train_epoch(args, global_step, model, param_optimizer,
    ...
    rank_metrics = evaluate_rank(args, model, device, eval_examples, eval_rank_features, eval_rank_dataloader,
                                 logger, 'dev', args.n_para_predict, force_answer=False, write_pred=False)
    ...
    read_metrics = evaluate_read(args, model, device, eval_examples, eval_read_features, eval_rank_logits,
                                 eval_read_dataloader, logger, write_pred=False)
    f = open(log_path, "a")
    print("Ranker, step: {}, map: {:.3f}, mrr: {:.3f}, "
          "top_1: {:.3f}, top_3: {:.3f}, top_5: {:.3f}, top_7: {:.3f}"
          .format(global_step, rank_metrics['map'], rank_metrics['mrr'],
                  rank_metrics['top_1'], rank_metrics['top_3'], rank_metrics['top_5'], rank_metrics['top_7']),
          file=f)
    print("Reader, step: {}, em: {:.3f}, f1: {:.3f}"
          .format(global_step,  read_metrics['exact_match'], read_metrics['f1']), file=f)
    print(" ", file=f)
    f.close()
...
def run_rank_eval(args, global_step, model, device, eval_examples, eval_features, eval_dataloader,
                  logger, log_path, save_path, type, n_para, force_answer=False, write_pred=False):
    ...
    model.eval()
    metrics = evaluate_rank(args, model, device, eval_examples, eval_features, eval_dataloader, logger,
                            type, n_para, force_answer=force_answer, write_pred=write_pred, verbose_logging=True)
    f = open(log_path, "a")
...
def run_read_eval(args, global_step, model, device, eval_examples, eval_features, eval_rank_logits, eval_dataloader,
                  logger, log_path, save_path, type, write_pred=False):
    # restore from best checkpoint
    if save_path and os.path.isfile(save_path) and args.do_train:
        checkpoint = torch.load(save_path)
        model.load_state_dict(checkpoint['model'])
        ...
        logger.info("Loading model from finetuned checkpoint: '{}' (step {})"
                    .format(save_path, step))
    model.eval()
    metrics = evaluate_read(args, model, device, eval_examples, eval_features, eval_rank_logits,
...
def main():
    ...
    parser.add_argument("--data_dir", default=None, type=str, help="SQuAD data dir")
    ...
    parser.add_argument("--do_predict_open", default=False, action='store_true', help="Whether to run eval on the open dev set.")
    ...
    parser.add_argument("--n_best_size_rank", default=4, type=int,
    ...
    parser.add_argument("--n_para_train", default=8, type=int,
                        help="The total number of paragraph used in training.")
    parser.add_argument("--n_para_predict", default=10, type=int,
    ...
    parser.add_argument("--down_sample", default=False, action='store_true',
                        help="If true, use self attention to summarize the sequence for classification")
    ...
    if not args.do_train and not args.do_predict and not args.do_predict_open:
    ...
    model = BertForRankingAndReadingAndReranking(bert_config, args.num_hidden_rank)
    ...
    if args.do_train:
        ...
        eval_read_dataloader = read_eval_data(args, tokenizer, logger)
        ...
        logger.info("***** Running eval distillation *****")
        run_rank_eval(args, global_step, model, device, eval_examples, eval_rank_features, eval_rank_dataloader,
                      logger, log_path, save_path=None, type='test', n_para=args.n_para_predict,
    ...
    if args.do_predict:
        ...
        args.n_para_predict = 30
        args.n_best_size_rank = 8
        eval_examples, eval_rank_features, _, _, eval_rank_dataloader, _ = read_eval_data(args, tokenizer, logger)
        ...
        run_rank_eval(args, global_step, model, device, eval_examples, eval_rank_features, eval_rank_dataloader, logger,
                      log_path, save_path, type='test', n_para=args.n_para_predict, force_answer=False, write_pred=True)

        logger.info("***** Reconstruct pred data at {} *****".format(args.rank_pred_file))
        ...
        run_read_eval(args, global_step, model, device, eval_examples, eval_read_features, eval_rank_logits,
                      eval_read_dataloader, logger, log_path, save_path, type='test', write_pred=True)

    if args.do_predict_open:
        logger.info("***** Preparing prediction *****")
        args.n_para_predict = 30
        args.n_best_size_rank = 8
        eval_examples, eval_rank_features, _, _, eval_rank_dataloader, _ = read_eval_open_data(args, tokenizer, logger)

        logger.info("***** Running ranking prediction *****")
        run_rank_eval(args, global_step, model, device, eval_examples, eval_rank_features, eval_rank_dataloader, logger,
                      log_path, save_path, type='test_open', n_para=args.n_para_predict, force_answer=False, write_pred=True)
