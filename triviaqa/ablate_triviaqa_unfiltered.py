...
from triviaqa.build_span_corpus import TriviaQaUnfilteredDataset, TriviaQaSampleUnfilteredDataset
from triviaqa.preprocessed_corpus import preprocess_par, ExtractMultiParagraphsPerQuestion, TopTfIdf


def main():
    parser = argparse.ArgumentParser(description='Train a model on TriviaQA open')
    parser.add_argument("--debug", default=False, action='store_true', help="Whether to run in debug mode.")
    parser.add_argument("--data_dir", default="data/triviaqa/unfiltered", type=str, help="Triviaqa wiki data dir")
    ...
    if args.debug:
        corpus = TriviaQaSampleUnfilteredDataset()
    else:
        corpus = TriviaQaUnfilteredDataset()
